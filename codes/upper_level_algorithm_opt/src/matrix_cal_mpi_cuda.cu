...
int main(int argc, char* argv[]) { 
    ...
    while (!is_end) {
        ...
        */
        ...
        cudaMalloc((void**)&dm1, m * n * sizeof(double) / size);
        cudaMalloc((void**)&dm2, n * p * sizeof(double));
        cudaMalloc((void**)&danswer, m * p * sizeof(double) / size);

        cudaMemcpy(dm1, &m1_local[0], m * n * sizeof(double) / size, cudaMemcpyHostToDevice);
        cudaMemcpy(dm2, &m2[0], n * p * sizeof(double), cudaMemcpyHostToDevice);

        dim3 grid((p + 15) / 16, (m + 15) / 16);
        dim3 block(16, 16);

        matrixMultiply<<<grid, block>>>(dm1, dm2, danswer, m / size, n, p);

        cudaMemcpy(&answer_local[0], danswer, m * p * sizeof(double) / size, cudaMemcpyDeviceToHost);
